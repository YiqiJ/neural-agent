...
def shuffled_timepoint_full_session(mouse_id: str, sidx: int, rng: np.random.Generator) -> pd.DataFrame:
    """
    Implements full-session trial shuffling as described:
    For each (cell_idx, behavior), concatenate all trials and all time bins into a 1D
    vector in trial-major order (sorted by trial_idx, then sample_idx), apply a single
    random permutation, then reshape/truncate back to (trials x bins). We then extract
    the column corresponding to the requested time bin (sidx) and return a DataFrame
    with rows equivalent to read_mouse_timepoint(mouse_id, sidx).
    """
    # Load entire session for this mouse
    table = pq.read_table(
        DATA_PATH,
        filters=[("mouse_id", "==", mouse_id)],
        columns=["trial_idx", "cell_idx", "amplitude", "behavior", "sample_idx"],
    )
    df_all = table.to_pandas()

    # Determine bin ordering and index for sidx
    bins_sorted = np.sort(df_all["sample_idx"].unique())
    bins_sorted = bins_sorted.tolist()
    if sidx not in bins_sorted:
        raise ValueError(f"Requested sample_idx {sidx} not present for mouse {mouse_id}")
    n_bins = len(bins_sorted)
    s_offset = bins_sorted.index(sidx)

    # Global sort to ensure trial-major order within each group
    df_all = df_all.sort_values(["cell_idx", "behavior", "trial_idx", "sample_idx"], kind="mergesort")

    out_records = []
    # Iterate groups; groupby preserves sort order on a sorted frame with sort=False
    for (cell_id, beh), g in df_all.groupby(["cell_idx", "behavior"], sort=False):
        # If a group lacks samples for this sidx, skip
        if sidx not in g["sample_idx"].unique():
            continue
        vals = g["amplitude"].to_numpy()
        # Apply a single random permutation for this (cell, behavior)
        perm = rng.permutation(vals.size)
        vals_perm = vals[perm]
        # Extract the sidx column after reshaping to (n_trials, n_bins) in trial-major order.
        # Number of trials in this group can be computed from count of rows at this sidx
        g_sidx = g[g["sample_idx"] == sidx]
        trial_ids = g_sidx["trial_idx"].to_numpy()
        # Expect len(vals_perm) == len(trial_ids)*n_bins; guard against mismatch
        n_trials_est = len(trial_ids)
        if n_trials_est * n_bins != vals_perm.size:
            # Fallback: recompute n_bins from this group uniquely and re-evaluate s_offset
            bins_g = np.sort(g["sample_idx"].unique())
            bins_g = bins_g.tolist()
            if sidx not in bins_g:
                continue
            n_bins_g = len(bins_g)
            s_off_g = bins_g.index(sidx)
            # Using group-specific bins
            take_vals = vals_perm[s_off_g :: n_bins_g]
        else:
            take_vals = vals_perm[s_offset :: n_bins]
        # Sanity trim in case of minor mismatch
        take_vals = take_vals[: len(trial_ids)]

        # Build rows for this group's sidx
        out_records.append(
            pd.DataFrame(
                {
                    "trial_idx": trial_ids,
                    "cell_idx": np.full_like(trial_ids, cell_id),
                    "amplitude": take_vals,
                    "behavior": np.full_like(trial_ids, beh),
                }
            )
        )

    if not out_records:
        return pd.DataFrame(columns=["trial_idx", "cell_idx", "amplitude", "behavior"])  # empty

    df_tp_shuf = pd.concat(out_records, ignore_index=True)
    return df_tp_shuf
...
def main():
    ...
    for mouse_id in mice:
        ...
        for t_key, sidx in tp_bins.items():
            ...
            df_shuf = shuffled_timepoint_full_session(mouse_id, sidx, rng)
            n_rows = len(df_shuf)
